...
public class OVRDirectComposition : OVRCameraComposition
{
        ...
        : base(parentObject, mainCamera, cameraDevice, useDynamicLighting, depthQuality)
    {
        ...
        directCompositionCameraGameObject = new GameObject { name = "MRDirectCompositionCamera" };
    ...
    public override void Update(Camera mainCamera)
    {
        ...
        else
        {
            ...
            if (OVRPlugin.GetMixedRealityCameraInfo(0, out var extrinsics, out var intrinsics))
            {
                ...
                if (cameraInTrackingSpace) directCompositionCamera.transform.FromOVRPose(ComputeCameraTrackingSpacePose(extrinsics), true);
                else directCompositionCamera.transform.FromOVRPose(ComputeCameraWorldSpacePose(extrinsics));
            }
