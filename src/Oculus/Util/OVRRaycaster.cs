...
{
    [Tooltip("A world space pointer for this canvas")] public GameObject pointer;
    ...
    /// world objects
    /// </summary>
    [NonSerialized] List<RaycastHit> m_RaycastResults = new List<RaycastHit>();
    void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList, Ray ray, bool checkForBlocking)
    {
        ...
        for (var index = 0; index < m_RaycastResults.Count; index++)
        {
            ...
            if (appendGraphic)
            {
                var distance = Vector3.Distance(ray.origin, m_RaycastResults[index].worldPos);
    ...
    /// </summary>
    [NonSerialized] static readonly List<RaycastHit> s_SortedGraphics = new List<RaycastHit>();
    void GraphicRaycast(Canvas canvas, Ray ray, List<RaycastHit> results)
    {
        ...
        for (var i = 0; i < s_SortedGraphics.Count; ++i)
    ...
    public Vector2 GetScreenPosition(RaycastResult raycastResult) =>
        ...
        eventCamera.WorldToScreenPoint(raycastResult.worldPosition);
    ...
    {
        var inputModule = EventSystem.current.currentInputModule as OVRInputModule;
