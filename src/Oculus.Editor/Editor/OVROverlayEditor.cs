...
public class OVROverlayEditor : Editor
{
    ...
    bool sourceRectsVisible = false;
    bool destRectsVisible = false;

    Material _SrcRectMaterial;
    protected Material SrcRectMaterial
    {
        get
        {
            ...
            {
                var shaders = AssetDatabase.FindAssets("OVROverlaySrcRectEditor");
                if (shaders.Length > 0)
                {
                    var shader = (Shader)AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(shaders[0]), typeof(Shader));
                    if (shader != null)
    ...
    protected Material DestRectMaterial
    {
        get
        {
            ...
            {
                var shaders = AssetDatabase.FindAssets("OVROverlayDestRectEditor");
                if (shaders.Length > 0)
                {
                    var shader = (Shader)AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(shaders[0]), typeof(Shader));
                    if (shader != null)
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            var lastIsExternalSurface = overlay.isExternalSurface;
        ...
        }
        {
            ...
            {
                var tmp = new Texture[2];
                for (var i = 0; i < overlay.textures.Length; i++)
            ...
            var right = (Texture)EditorGUI.ObjectField(new Rect(textureControlRect.x + textureControlRect.width / 2, textureControlRect.y, 64, textureControlRect.height), overlay.textures[1] != null ? overlay.textures[1] : overlay.textures[0], typeof(Texture), true);
            if (right == overlay.textures[0]) overlay.textures[1] = null;
            else overlay.textures[1] = right;
    ...
    label1:
        if (overlay.currentOverlayShape == OVROverlay.OverlayShape.Cylinder || overlay.currentOverlayShape == OVROverlay.OverlayShape.Equirect)
        {
            ...
            var lastOverrideTextureRectMatrix = overlay.overrideTextureRectMatrix;
            ...
            if (lastOverrideTextureRectMatrix)
            {
                ...
                if (sourceRectsVisible)
                {
                    ...
                    if (mat != null)
                    {
                        var drawRect = EditorGUILayout.GetControlRect(GUILayout.Height(128 + 8));
        ...
        {
            var colorScale = EditorGUILayout.Vector4Field(new GUIContent("Color Scale", "Scale that the color values for this overlay will be multiplied by."), overlay.colorScale);
            var colorOffset = EditorGUILayout.Vector4Field(new GUIContent("Color Offset", "Offset that the color values for this overlay will be added to."), overlay.colorOffset);
            overlay.SetPerLayerColorScaleAndOffset(colorScale, colorOffset);
        }
        EditorUtility.SetDirty(overlay);
    }

    Rect Clamp01(Rect rect)
    {
    ...
    bool IsUnitRect(Rect rect) => IsRect(rect, 0, 0, 1, 1);
    bool IsRect(Rect rect, float x, float y, float w, float h) => rect.x == x && rect.y == y && rect.width == w && rect.height == h;

    StereoType GetStereoType(OVROverlay overlay)
    {
        if (overlay.textures[0] != null && overlay.textures[1] != null)
            return IsUnitRect(overlay.srcRectLeft) && IsUnitRect(overlay.srcRectRight) ? StereoType.Stereo : StereoType.Custom;
        else if (overlay.textures[0] != null)
        {
            if (IsUnitRect(overlay.srcRectLeft) && IsUnitRect(overlay.srcRectRight)) return StereoType.Mono;
            else if (IsRect(overlay.srcRectLeft, 0, 0, 0.5f, 1f) && IsRect(overlay.srcRectRight, 0.5f, 0, 0.5f, 1f)) return StereoType.StereoLeftRight;
            else if (IsRect(overlay.srcRectLeft, 0, 0, 1f, 0.5f) && IsRect(overlay.srcRectRight, 0f, 0.5f, 1f, 0.5f)) return StereoType.StereoTopBottom;
            else return StereoType.Custom;
        }
        else return StereoType.Mono;
    }

    void SetRectsByVideoType(OVROverlay overlay, StereoType stereoType, DisplayType displayType)
    {
        Rect srcRectLeft, srcRectRight, destRectLeft, destRectRight;
        switch (displayType)
        {
            case DisplayType.Full: destRectLeft = destRectRight = new Rect(0, 0, 1, 1); break;
            case DisplayType.Half: destRectLeft = destRectRight = new Rect(0.25f, 0, 0.5f, 1); break;
            default: destRectLeft = overlay.destRectLeft; destRectRight = overlay.destRectRight; break;
        }
        switch (stereoType)
        {
            case StereoType.Mono:
            case StereoType.Stereo: srcRectLeft = srcRectRight = new Rect(0, 0, 1, 1); break;
            case StereoType.StereoTopBottom: srcRectLeft = new Rect(0, 0, 1, 0.5f); srcRectRight = new Rect(0, 0.5f, 1, 0.5f); break;
            case StereoType.StereoLeftRight: srcRectLeft = new Rect(0, 0, 0.5f, 1); srcRectRight = new Rect(0.5f, 0, 0.5f, 1); break;
            default: srcRectLeft = overlay.srcRectLeft; srcRectRight = overlay.srcRectRight; break;
        }
        overlay.SetSrcDestRects(srcRectLeft, srcRectRight, destRectLeft, destRectRight);
    }

    void GetCursorPoints(Rect drawRect, Rect selectRect, out Vector2 leftPos, out Vector2 rightPos, out Vector2 topPos, out Vector2 bottomPos)
    {
    ...
    void CreateCursorRects(Rect drawRect, Rect selectRect)
    {
        GetCursorPoints(drawRect, selectRect, out var leftPos, out var rightPos, out var topPos, out var bottomPos);
    ...
    bool IsOverRectControls(Rect drawRect, Vector2 mousePos, Rect selectRect, ref Side side)
    {
        GetCursorPoints(drawRect, selectRect, out var leftPos, out var rightPos, out var topPos, out var bottomPos);
        if ((leftPos - mousePos).sqrMagnitude <= 25) { side = Side.Left; return true; }
        if ((rightPos - mousePos).sqrMagnitude <= 25) { side = Side.Right; return true; }
        if ((topPos - mousePos).sqrMagnitude <= 25) { side = Side.Top; return true; }
        if ((bottomPos - mousePos).sqrMagnitude <= 25) { side = Side.Bottom; return true; }
        return false;
    }

    void UpdateRectDragging(Rect drawingRectLeft, Rect drawingRectRight, TextureRect rectLeftType, TextureRect rectRightType, ref Rect rectLeft, ref Rect rectRight)
    {
    ...
    void SetRectSideValue(Rect drawingRect, Vector2 mousePos, Side side, ref Rect rect)
    {
        // quantize to 1/32
        var x = Mathf.Clamp01(Mathf.Round(((mousePos.x - drawingRect.x) / drawingRect.width) * 32) / 32.0f);
        var y = Mathf.Clamp01(Mathf.Round(((mousePos.y - drawingRect.y) / drawingRect.height) * 32) / 32.0f);
        switch (side)
        {
            case Side.Left: var xMax = rect.xMax; rect.x = Mathf.Min(x, xMax); rect.width = xMax - rect.x; break;
            case Side.Right: rect.width = Mathf.Max(0, x - rect.x); break;
            case Side.Top: var yMax = rect.yMax; rect.y = Mathf.Min(y, yMax); rect.height = yMax - rect.y; break;
            case Side.Bottom: rect.height = Mathf.Max(0, y - rect.y); break;
        }
    }
}
