...
public class OVRManagerEditor : Editor
{
    override public void OnInspectorGUI()
    {
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            var oldTargetDeviceTypes = projectConfig.targetDeviceTypes;
            var targetDeviceTypes = new List<OVRProjectConfig.DeviceType>(oldTargetDeviceTypes);
            var hasModified = false;
            var newCount = Mathf.Max(0, EditorGUILayout.IntField("Size", targetDeviceTypes.Count));
            ...
            for (var i = 0; i < targetDeviceTypes.Count; i++)
            {
        ...
        {
            var layerMaskOptions = new string[32];
            for (var i = 0; i < 32; ++i)
            {
    ...
    void SetupBoolField(string name, ref bool member)
    {
        ...
        var value = EditorGUILayout.Toggle(name, member);
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); member = value; }
    }
    void SetupIntField(string name, ref int member)
    {
        EditorGUI.BeginChangeCheck();
        var value = EditorGUILayout.IntField(name, member);
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); member = value; }
    }
    void SetupFloatField(string name, ref float member)
    {
        EditorGUI.BeginChangeCheck();
        var value = EditorGUILayout.FloatField(name, member);
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); member = value; }
    }
    void SetupDoubleField(string name, ref double member)
    {
        EditorGUI.BeginChangeCheck();
        var value = EditorGUILayout.DoubleField(name, member);
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); member = value; }
    }
    void SetupColorField(string name, ref Color member)
    {
        EditorGUI.BeginChangeCheck();
        var value = EditorGUILayout.ColorField(name, member);
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); member = value; }
    }
    void SetupLayerMaskField(string name, ref LayerMask layerMask, string[] layerMaskOptions)
    {
        EditorGUI.BeginChangeCheck();
        var value = EditorGUILayout.MaskField(name, layerMask, layerMaskOptions);
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); layerMask = value; }
    }
    void SetupCompositoinMethodField(string name, ref OVRManager.CompositionMethod method)
    {
        ...
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); method = value; }
    }
    void SetupCameraDeviceField(string name, ref OVRManager.CameraDevice device)
    {
        ...
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); device = value; }
    }
    void SetupDepthQualityField(string name, ref OVRManager.DepthQuality depthQuality)
    {
        ...
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); depthQuality = value; }
    }
    void SetupVirtualGreenTypeField(string name, ref OVRManager.VirtualGreenScreenType virtualGreenScreenType)
    {
        ...
        if (EditorGUI.EndChangeCheck()) { Undo.RecordObject(target, "Changed " + name); virtualGreenScreenType = value; }
    }
#endif
}
