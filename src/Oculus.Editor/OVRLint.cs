...
public class OVRLint : EditorWindow
{
    ...
    static List<FixRecord> mRecords = new List<FixRecord>();
    static List<FixRecord> mRuntimeEditModeRequiredRecords = new List<FixRecord>();
    static bool mWasPlaying = false;
    ...
    static void Init()
    {
        ...
        GetWindow(typeof(OVRLint));
        OVRPlugin.SendEvent("perf_lint", "activated");
        RunCheck();
    }

    OVRLint() => EditorApplication.playModeStateChanged += HandlePlayModeState;

    static void HandlePlayModeState(PlayModeStateChange state)
    {
        if (state == PlayModeStateChange.EnteredEditMode)
            ApplyEditModeRequiredFix();
    }

    static void ApplyEditModeRequiredFix()
    {
        // Apply runtime fixes that require edit mode when applying fix
        foreach (var record in mRuntimeEditModeRequiredRecords)
        {
    ...
    void OnGUI()
    {
        ...
        for (var x = 0; x < mRecords.Count; x++)
        {
            var record = mRecords[x];
            if (!record.category.Equals(lastCategory))  // new category
            {
                ...
                var moreThanOne = x + 1 < mRecords.Count && mRecords[x + 1].category.Equals(lastCategory);
                if (record.buttonNames != null && record.buttonNames.Length > 0 && moreThanOne)
                {
                    GUILayout.Label("Apply to all:", EditorStyles.label, GUILayout.Width(75));
                    for (var y = 0; y < record.buttonNames.Length; y++)
                        if (GUILayout.Button(record.buttonNames[y], EditorStyles.toolbarButton, GUILayout.Width(200)))
                        {
                            var recordsToProcess = new List<FixRecord>();
                            for (var z = x; z < mRecords.Count; z++)
                            {
                                var thisRecord = mRecords[z];
                                var isLast = false;
                            ...
                            var undoObjects = new Object[recordsToProcess.Count];
                            for (var z = 0; z < recordsToProcess.Count; z++)
                                undoObjects[z] = recordsToProcess[z].targetObject;
                            Undo.RecordObjects(undoObjects, record.category + " (Multiple)");
                            for (var z = 0; z < recordsToProcess.Count; z++)
                            {
                                var thisRecord = recordsToProcess[z];
            ...
            if (record.targetObject) EditorGUILayout.ObjectField(record.targetObject, record.targetObject.GetType(), true);
            else GUILayout.Label(record.message);
    ...
    static void CheckStaticCommonIssues()
    {
        ...
        var recommendedPixelLightCount = Application.platform == RuntimePlatform.Android ? 1 : 3;
		...
		if (!PlayerSettings.graphicsJobs)
			AddFix ("Optimize Graphics Jobs", "For CPU performance, please use graphics jobs.", delegate(Object obj, bool last, int selected)
			{
        ...
        if (Application.platform == RuntimePlatform.Android)
            if (!PlayerSettings.use32BitDisplayBuffer)
                AddFix("Optimize Display Buffer Format", "We recommend to enable use32BitDisplayBuffer.", delegate (Object obj, bool last, int selected)
                {
        ...
        if (Application.platform != RuntimePlatform.Android)
        {
            if (!PlayerSettings.VROculus.dashSupport)
                AddFix("Enable Dash Integration", "We recommend to enable Dash Integration for better user experience.", delegate (Object obj, bool last, int selected)
                {
                    PlayerSettings.VROculus.dashSupport = true;
                }, null, false, "Fix");
            if (!PlayerSettings.VROculus.sharedDepthBuffer)
                AddFix("Enable Depth Buffer Sharing", "We recommend to enable Depth Buffer Sharing for better user experience on Oculus Dash.", delegate (Object obj, bool last, int selected)
                {
        ...
        if (tierSettings.renderingPath == RenderingPath.DeferredShading || tierSettings.renderingPath == RenderingPath.DeferredLighting)
        ...
        {
            var playingAudioSources = new List<AudioSource>();
            ...
            if (playingAudioSources.Count > 16)
            {
                ...
                for (var i = 16; i < playingAudioSources.Count; ++i)
        ...
        var clips = FindObjectsOfType<AudioClip>();
        for (var i = 0; i < clips.Length; ++i)
        {
            if (clips[i].loadType == AudioClipLoadType.DecompressOnLoad)
                ...
                {
                    var thisClip = (AudioClip)obj;
                    if (selected == 0) SetAudioLoadType(thisClip, AudioClipLoadType.CompressedInMemory, last);
                    else SetAudioLoadType(thisClip, AudioClipLoadType.Streaming, last);
            ...
            if (materials[i].shader.name.Contains("Parallax") || materials[i].IsKeywordEnabled("_PARALLAXMAP"))
                AddFix("Optimize Shading", "For GPU performance, please don't use parallax-mapped materials.", delegate (Object obj, bool last, int selected)
                {
                    ...
                    if (thisMaterial.shader.name.Contains("Parallax"))
                    {
                        ...
                        if (newShader) thisMaterial.shader = newShader;
                        else Debug.LogWarning("Unable to find a replacement for shader " + materials[i].shader.name);
                    }
        ...
        var overlays = FindObjectsOfType<OVROverlay>();
        if (overlays.Length > 4)
            ...
            {
                for (var i = 4; i < OVROverlay.instances.Length; ++i)
    ...
    static void CheckRuntimeCommonIssues()
    {
        ...
        if (OVRManager.instance != null && !OVRManager.instance.useRecommendedMSAALevel)
            ...
            {
                var ovrManagers = FindObjectsOfType<OVRManager>();
    ...
    {
        var recommendedAndroidSdkVersion = AndroidSdkVersions.AndroidApiLevel21;
        if ((int)PlayerSettings.Android.minSdkVersion < (int)recommendedAndroidSdkVersion)
            AddFix("Optimize Android API Level", "To avoid legacy workarounds, please require at least API level " + (int)recommendedAndroidSdkVersion, delegate (Object obj, bool last, int selected)
            {
                PlayerSettings.Android.minSdkVersion = recommendedAndroidSdkVersion;
            }, null, false, "Fix");
        if (!PlayerSettings.gpuSkinning)
            AddFix("Optimize GPU Skinning", "If you are CPU-bound, consider using GPU skinning.", delegate (Object obj, bool last, int selected)
            {
                PlayerSettings.gpuSkinning = true;
            }, null, false, "Fix");
        if (RenderSettings.skybox)
            AddFix("Optimize Clearing", "For GPU performance, please don't use Unity's built-in Skybox.", delegate (Object obj, bool last, int selected)
            {
        ...
        for (var i = 0; i < materials.Length; ++i)
        {
            if (materials[i].IsKeywordEnabled("_SPECGLOSSMAP") || materials[i].IsKeywordEnabled("_METALLICGLOSSMAP"))
                AddFix("Optimize Specular Material", "For GPU performance, please don't use specular shader on materials.", delegate (Object obj, bool last, int selected)
                {
                    var thisMaterial = (Material)obj;
        ...
        if (backend != ScriptingImplementation.IL2CPP)
            AddFix("Optimize Scripting Backend", "For CPU performance, please use IL2CPP.", delegate (Object obj, bool last, int selected)
            {
                PlayerSettings.SetScriptingBackend(BuildTargetGroup.Android, ScriptingImplementation.IL2CPP);
            }, null, false, "Fix");

        var monoBehaviours = FindObjectsOfType<MonoBehaviour>();
        var effectBaseType = System.Type.GetType("UnityStandardAssets.ImageEffects.PostEffectsBase");
        ...
        var maxTextureSize = 1024 * (1 << QualitySettings.masterTextureLimit);
        maxTextureSize = maxTextureSize * maxTextureSize;
        for (var i = 0; i < textures.Length; ++i)
            if (textures[i].filterMode == FilterMode.Trilinear && textures[i].mipmapCount == 1)
                AddFix("Optimize Texture Filtering", "For GPU performance, please generate mipmaps or disable trilinear filtering for textures.", delegate (Object obj, bool last, int selected)
                {
                    var thisTexture = (Texture2D)obj;
                    if (selected == 0) thisTexture.filterMode = FilterMode.Bilinear;
                    else SetTextureUseMips(thisTexture, true, last);
                }, textures[i], false, "Switch to Bilinear", "Generate Mipmaps");

        var projectors = FindObjectsOfType<Projector>();
        if (projectors.Length > 0)
            AddFix("Optimize Projectors", "For GPU performance, please don't use projectors.", delegate (Object obj, bool last, int selected)
            {
                var thisProjectors = FindObjectsOfType<Projector>();
                for (var i = 0; i < thisProjectors.Length; ++i)
        ...
        if (EditorUserBuildSettings.androidBuildSubtarget != MobileTextureSubtarget.ASTC)
            AddFix("Optimize Texture Compression", "For GPU performance, please use ASTC.", delegate (Object obj, bool last, int selected)
            {
        ...
        for (var i = 0; i < cameras.Length; ++i)
            if (cameras[i].forceIntoRenderTexture)
                ...
                delegate (Object obj, bool last, int selected)
                {
    ...
    static void CheckRuntimeAndroidIssues()
    {
        ...
        if (OVRManager.cpuLevel < 0 || OVRManager.cpuLevel > 3)
            AddFix("Optimize CPU level", "For battery life, please use a safe CPU level.", delegate (Object obj, bool last, int selected)
            {
                OVRManager.cpuLevel = 2;
            }, null, false, "Set to CPU2");
        if (OVRManager.gpuLevel < 0 || OVRManager.gpuLevel > 3)
            AddFix("Optimize GPU level", "For battery life, please use a safe GPU level.", delegate (Object obj, bool last, int selected)
            {
    ...
    static bool IsLightBaked(Light light) => light.lightmapBakeType == LightmapBakeType.Baked;
